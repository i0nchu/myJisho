"""Dependency-free HTTP API for myJisho's self-hosted local dictionary."""
...
class LocalDictionaryHTTPServer(ThreadingHTTPServer):
    ...
    def __init__(
        ...
        *,
        ...
        self.worker_pool = ThreadPoolExecutor(max_workers=2, thread_name_prefix="myjisho-generate")
...
class LocalDictionaryHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if route.path == "/api/health":
            self._json(
                ...
                {
                    "ok": True,
                    "service": "myjisho-local-dictionary",
    ...
    def _request_origin(self) -> tuple[str, str, int]:
        ...
        if hostname not in self.server.allowed_hosts:
            raise ValueError("Host is not allowed for this server")
        if not is_trusted_proxy_request and port is not None:
            effective_port = port
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the myJisho self-hosted local dictionary API")
    parser.add_argument(
        "--host",
        default=os.environ.get("MYJISHO_API_HOST", "127.0.0.1"),
    )
    ...
    parser.add_argument(
        "--api-token",
        default=os.environ.get("MYJISHO_API_TOKEN", ""),
    ...
    env_allowed_hosts = {
        item.strip()
        for item in os.environ.get("MYJISHO_ALLOWED_HOSTS", "").split(",")
        if item.strip()
    }
    try:
    ...
    generator = DictionaryGenerator(WikimediaSearchProvider(), llm)
    ...
    print(f"myJisho local dictionary API: http://{args.host}:{server.server_port}")
