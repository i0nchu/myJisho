...
class LLMConfigurationError(ValueError):
    """Raised when the configured LLM endpoint cannot be initialized."""
    ...
    def _search_endpoint(
        ...
        request = Request(
            ...
            headers={
                ...
                "User-Agent": "myJishoSelfHosted/0.1.1 (local Japanese dictionary)",
            },
        )
...
class OpenAICompatibleLLMProvider:
    ...
    def __init__(
        ...
        *,
        ...
        self.base_url = (base_url or os.environ.get("MYJISHO_LLM_BASE_URL") or "http://127.0.0.1:11434/v1").rstrip("/")
        configured_model = (
            model if model is not None else os.environ.get("MYJISHO_LLM_MODEL")
        )
        if configured_model is None or not configured_model.strip():
            raise LLMConfigurationError(
                "MYJISHO_LLM_MODEL is required; set it to a model exposed by "
                "the configured OpenAI-compatible endpoint"
            )
        self._model = configured_model.strip()
        self.api_key = api_key if api_key is not None else os.environ.get("MYJISHO_LLM_API_KEY", "")
        self.timeout_seconds = timeout_seconds
