...
class LocalDictionaryServerTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(prefix="myjisho-local-api-")
        ...
        self.server = create_server(
            ...
            allowed_hosts={"myjisho-stage.example.ts.net", "100.100.123.123"},
        )
    ...
    def test_private_https_reverse_proxy_host_and_origin_are_allowed(self) -> None:
        status, payload = self.request(
            ...
            {"query": "食べる"},
            headers={
                "Host": "myjisho-stage.example.ts.net",
                "Origin": "https://myjisho-stage.example.ts.net",
            },
        )
        self.assertEqual(status, 202)
    ...
    def test_tailscale_forwarded_host_and_origin_are_allowed(self) -> None:
        status, payload = self.request(
            ...
            headers={
                "Origin": "https://myjisho-stage.example.ts.net",
                "X-Forwarded-Host": "myjisho-stage.example.ts.net",
    ...
    def test_direct_tailscale_ip_and_origin_are_allowed(self) -> None:
        origin = f"http://100.100.123.123:{self.server.server_port}"
        status, payload = self.request(
            "POST",
            "/api/generation-jobs",
            {"query": "食べる"},
            headers={
                "Host": f"100.100.123.123:{self.server.server_port}",
                "Origin": origin,
            },
        )
        self.assertEqual(status, 202)
    ...
    def test_reverse_proxy_host_rejects_non_https_port(self) -> None:
        status, payload = self.request(
            "GET",
            "/api/health",
            headers={"Host": "myjisho-stage.example.ts.net:80"},
        )
