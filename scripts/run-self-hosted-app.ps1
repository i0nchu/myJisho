...
foreach ($line in Get-Content -LiteralPath $environmentFile -Encoding utf8) {
    if ($line -match '^KOTOBA_API_TOKEN=(.+)$') {
        $token = $Matches[1]
        break
    }
}
if (-not $token) {
    throw 'KOTOBA_API_TOKEN is missing from deploy/.env.'
}
...
try {
    & $flutter run `
        -d $Device `
        "--dart-define=KOTOBA_LOCAL_API=$ApiUrl" `
        "--dart-define=KOTOBA_LOCAL_API_TOKEN=$token"
