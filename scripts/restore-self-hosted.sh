...
BACKUP_FILE 必須位於 deploy/.env 的 MYJISHO_BACKUP_DIR 內。
...
deployment_mode=$(awk -F= '$1 == "MYJISHO_DEPLOYMENT_MODE" { sub(/^[^=]*=/, ""); print; exit }' "${env_file}")
if [[ ${deployment_mode:-internal} != "internal" ]]; then
  echo "此還原工具只支援目前的 internal deployment mode。" >&2
  exit 1
fi

backup_setting=$(awk -F= '$1 == "MYJISHO_BACKUP_DIR" { sub(/^[^=]*=/, ""); print; exit }' "${env_file}")
...
if [[ ! ${archive_name} =~ ^myjisho-stage-[0-9]{8}T[0-9]{6}Z\.tar\.gz$ ]]; then
...
"${compose[@]}" stop -t 30 myjisho-api
restart_api() {
  "${compose[@]}" up -d myjisho-api >/dev/null
}
