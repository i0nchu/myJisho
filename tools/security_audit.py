"""myJisho release supply-chain audit using only the Python standard library."""
...
SCHEMA_VERSION = "myjisho-security-audit-v1"
    ...
    *,
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "myJisho-security-audit/1",
        },
        method="POST",
    )
    try:
    ...
    *,
    ...
    for package in packages:
        component: dict[str, object] = {
            ...
            "properties": [
                {"name": "myjisho:pub-source", "value": package.source},
                {"name": "myjisho:dependency-kind", "value": package.dependency},
            ],
        }
    ...
    serial = uuid.uuid5(uuid.NAMESPACE_URL, f"myjisho:{lock_sha256}")
    return {
        "bomFormat": "CycloneDX",
        "specVersion": "1.7",
        ...
        "metadata": {
            ...
            "component": {
                "type": "application",
                "bom-ref": f"pkg:generic/myjisho_dictionary_app@{urllib.parse.quote(app_version)}",
                "name": "myjisho_dictionary_app",
                "version": app_version,
                "purl": f"pkg:generic/myjisho_dictionary_app@{urllib.parse.quote(app_version)}",
            },
            "properties": [
                {"name": "myjisho:pubspec-lock-sha256", "value": lock_sha256}
            ],
        },
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sbom", type=Path, default=Path("build/myjisho.cdx.json")
    )
